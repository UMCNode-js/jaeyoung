...
import { connectFoodCategory, confirmEmail, getUserID, insertUserSql, getPreferToUserID, insertMemberMission,getMemberMissionID } from "./user.sql.js";
...
export const addMemberMission = async (data) => {
    try{
        ...
        // 멤버 미션 추가
        const result = await pool.query(insertMemberMission, [data.mission_id, data.member_id]);
...
export const getMemberMission = async (membermissionId) => {
    try {
        const conn = await pool.getConnection();
        const [membermission] = await pool.query(getMemberMissionID, membermissionId);
